...
public class ConfigAction {
    ...
    public static void init() {
        InputEvent.MOUSE_BUTTON_PRE.register(ConfigAction::mouseAction);
        InputEvent.MOUSE_SCROLL.register(ConfigAction::wheelAction);
    }


    public static boolean mouseAction(int button, int action, int modifiers) {
        if (action != InputConstants.PRESS) return false;
        if (Minecraft.getInstance().player == null) return false;
        if (!KeyUtils.isKeyPressed(ModKeybindings.CONTROL_KEY.key.getValue())) return false;
